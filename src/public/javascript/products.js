...
/*
    ...
    if(coordinatesArrayBestellt.length > 0 || coordinatesArrayImWarenkorb.length > 0){ 
      ...
      for (var i = 0; i < coordinatesArrayBestellt.length; i++) {
          var latitude = coordinatesArrayBestellt[i][1];
          var longitude = coordinatesArrayBestellt[i][2];
      ...
      for (var i = 0; i < coordinatesArrayImWarenkorb.length; i++) {
        var latitude = coordinatesArrayImWarenkorb[i][1];
        var longitude = coordinatesArrayImWarenkorb[i][2];
  ...
  L.tileLayer('https://{s}.google.com/vt/lyrs=s,h&x={x}&y={y}&z={z}', {
    ...
    //if(userFlächen.products.length > 0){
...
function readDataFromKMLFile(file) {
  ...
  fileReader.onload = async (e) => {
    ...
    console.log(information.features[0].geometry.coordinates)
    //gotToSever();
    ...
    shapefile
      ...
      .then((source) => {
        source.read().then(function log(result) {
          ...
          if (result.done) {
            console.log('DORT');
            console.log('Hier: ' + JSON.stringify(fc));
            onHightLight(fc);
            return;
          }
        ...
        source.read().then(function log(result) {
          ...
          result.value.coordinates.forEach(coord => {
            for (let i = 0; i < coord.length; i++) {
              console.log(i+1 + ': ' + coord[i]);
              const wgs84Coordinates = proj4(gaussKrueger, wgs84, coord[i]);
              console.log('WGS84-Koordinadddten:', wgs84Coordinates);
...
function updateProjectionInPRJFile(prjContent) {
    ...
    if (prjContent.includes('PROJECTION["Gauss_Kruger"]')) {
        ...
        console.log('Die Projektion wurde geändert.');
    } else {
        console.log('Die PRJ-Datei enthält bereits eine andere Projektion oder keine Projektion.');
    }
